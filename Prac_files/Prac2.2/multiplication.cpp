...
#include<stdio.h>
#include<OpenCL/cl.h>//look out for this
#include<iostream>
#include<fstream>
#include<string>
#include<cmath>
...
int main(void)
{
    ...
    clock_t start_overhead, end_overhead, start_total, end_total, start_matrix, end_matrix;  //create Timers
    start_total = clock(); //start timer for total exectution time
    ...
    cl_mem matrixA_buffer, matrixB_buffer, output_buffer, size_buffer; //create the buffers
    ...
    sizeof(int)*Size*Size, matrixA, &err); //buffer for matrixA
    
    matrixB_buffer = clCreateBuffer(context,
    CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
    sizeof(int)*Size*Size, matrixB, &err); //buffer for matrix B
    
    output_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, global_size*sizeof(int), output, &err); //output buffer
    ...
    end_overhead = clock(); //end timer for overhead and display value
    printf ("Run Time overhead: %0.8f sec \n",((float) end_overhead - start_overhead)/CLOCKS_PER_SEC);
    
    start_matrix = clock(); //start timer for matrix multiplication
    ...
    end_matrix = clock(); //end timer for matrix multiplication and display value
  ...
  if(displayMatrices){
    ...
    end_total = clock(); //end timer for exectution time and siplay value
